...
st.title("Compare Stat with Session")
...
if session_file and feeder_file and stat_file:
    ...
    filtered_feeder = common.filter_by_session_window(feeder_df, common.FEEDER_TIME_COL, session_df)
    filtered_stat = common.filter_by_session_window(stat_df, common.STAT_SRC_TIME_COL, session_df)

    result = common.compare(filtered_feeder, filtered_stat)
    common.render_comparison(result)
else:
    st.info("Upload session.csv, a feeder log, and a stat-cap log CSV to compare within session windows.")
